...
export default function WhatsAppFAB({ currentLang, activeView }: WhatsAppFABProps) {
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden="true"
        className={`pointer-events-none absolute top-1/2 hidden -translate-y-1/2 whitespace-nowrap rounded-xl border border-brand-border bg-brand-card/95 px-3 py-2 text-[10px] font-bold text-brand-text opacity-0 shadow-xl backdrop-blur-md transition-all group-hover:opacity-100 md:block ${
          isRtl
            ? "left-[calc(100%+0.75rem)] -translate-x-1 group-hover:translate-x-0"
            : "right-[calc(100%+0.75rem)] translate-x-1 group-hover:translate-x-0"
        }`}
      >
        {label}
      </span>
      ...
      <span
        aria-hidden="true"
        className="absolute -right-1 -top-1 h-3.5 w-3.5 rounded-full border-2 border-brand-bg bg-amber-400 shadow-sm"
      />
    </motion.a>
  );
}
